...
export const AdminPanel: React.FC<AdminPanelProps> = ({
  ...
  const handleLogin = (e: React.FormEvent) => {
    e.preventDefault();
    // Default PIN: admin123 or whatever configured in settings
    const validPass = settings.adminPasswordHash || 'admin123';
    if (passwordInput === validPass || passwordInput === 'admin') {
      setIsAuthenticated(true);
      sessionStorage.setItem('pedalae_admin_auth', 'true');
      setAuthError('');
    } else {
      setAuthError('Senha incorreta. (Senha padrão: admin123)');
    }
  };
  ...
  const handleConfirmPayment = async (reservation: Reservation) => {
    if (confirm(`Confirmar recebimento do pagamento da reserva ${reservation.code}?`)) {
      await updateReservationStatus(reservation.id, 'pagamento_confirmado', {
        ...
        paymentConfirmedBy: 'Administrador Pedalaê',
      });
      // Refresh current detail modal if open
      setSelectedResForDetail((prev) =>
        prev && prev.id === reservation.id
          ? { ...prev, status: 'pagamento_confirmado', paymentConfirmedAt: Date.now() }
          : prev
      );
    }
  };
      ...
      <div className="min-h-screen bg-black flex items-center justify-center p-4">
        <div className="w-full max-w-md bg-zinc-900 border border-zinc-800 rounded-3xl p-8 shadow-2xl relative">
          ...
          <div className="flex flex-col items-center text-center mb-6">
            <img src="/logo.png" alt="PEDALAÊ" className="w-20 h-20 rounded-full border border-amber-500 mb-3" />
            <h2 className="text-xl font-black text-white">Painel Administrativo</h2>
            <p className="text-xs text-zinc-400 mt-1">
              Acesso exclusivo para gerentes e operadores do PEDALAÊ
            </p>
          </div>
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full bg-black border border-zinc-700 rounded-xl px-4 py-3 text-white text-sm focus:outline-none focus:border-amber-400"
                />
              </div>
            </div>
            ...
            <button
              ...
              type="submit"
              className="w-full py-3.5 rounded-xl bg-amber-500 hover:bg-amber-400 text-black font-extrabold text-sm uppercase tracking-wider transition-all shadow-lg shadow-amber-500/20"
            >
              Entrar no Painel
            </button>
          </form>
      ...
      <aside className="w-full md:w-64 bg-zinc-950 border-r border-zinc-900 flex flex-col justify-between flex-shrink-0">
        ...
        <div className="p-3 border-t border-zinc-900 space-y-2">
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 py-2 px-3 rounded-xl text-rose-400 hover:bg-rose-500/10 text-xs font-semibold"
          >
                        ...
                        <div className="flex items-center gap-2 self-end sm:self-auto">
                          ...
                          {res.status === 'bike_em_uso' && (
                            <button
                              ...
                              className="bg-purple-600 hover:bg-purple-500 text-white text-xs font-bold px-3 py-1.5 rounded-lg"
                            >
                              Registrar Devolução
                            </button>
                          )}
                          ...
                          <button
                            ...
                            className="bg-zinc-800 hover:bg-zinc-700 text-zinc-300 text-xs font-semibold px-2.5 py-1.5 rounded-lg"
                          >
                            Detalhes
                          </button>
                        </div>
                      ...
                      filteredReservations.map((res) => (
                        <tr key={res.id} className="hover:bg-zinc-850/50">
                          ...
                          <td className="py-3 px-4 text-right space-x-1.5 whitespace-nowrap">
                            ...
                            <button
                              onClick={() => generateReservationPDF(res, settings)}
              ...
              <div className="flex flex-wrap gap-2 pt-2 border-t border-zinc-800">
                ...
                {selectedResForDetail.status === 'pagamento_confirmado' && (
                  <button
                    ...
                    className="flex-1 py-2.5 px-4 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-bold text-xs"
                  >
                    Iniciar Check-in / Retirada
                  </button>
                )}
                ...
                {selectedResForDetail.status === 'bike_em_uso' && (
                  <button
                    ...
                    className="flex-1 py-2.5 px-4 rounded-xl bg-purple-600 hover:bg-purple-500 text-white font-bold text-xs"
                  >
                    Iniciar Devolução
                  </button>
                )}
                ...
                <button
                  onClick={() => generateReservationPDF(selectedResForDetail, settings)}
                  className="py-2.5 px-4 rounded-xl bg-amber-500 hover:bg-amber-400 text-black font-extrabold text-xs flex items-center gap-1.5"
                >
                ...
                {selectedResForDetail.status !== 'cancelada' && (
                  <button
                    ...
                    className="py-2.5 px-3 rounded-xl bg-rose-500/20 hover:bg-rose-500/30 text-rose-300 font-bold text-xs"
                  >
                    Cancelar Reserva
                  </button>
                )}
